...
class AqaraOAuth2Client extends OAuth2Client {
  ...
  async onRequestHeaders({ headers }) {
    ...
    if (!token || !token.access_token) {
      throw new OAuth2Error('Aqara authorization is missing. Connect your Aqara account first.');
    }

    const app = this.homey.app;
    const keyId = typeof app.getAqaraKeyId === 'function' ? app.getAqaraKeyId() : '';
    const appId = this._clientId;
    const appKey = this._clientSecret;

    if (!keyId || !appId || !appKey) {
      throw new OAuth2Error('Aqara developer credentials are not configured in Homey app settings.');
    }
  ...
  async getAllDevices() {
    ...
    while (true) {
      ...
      if (page.length === 0 || page.length < pageSize || (totalCount > 0 && devices.length >= totalCount)) {
        break;
      }

      pageNum += 1;
      if (pageNum > 100) {
        throw new OAuth2Error('Aqara device pagination exceeded the safety limit.');
      }
    }
