...
/** Aqara account authorization without an OAuth redirect URI. */
class AqaraAccountAuthClient {
  ...
  getCredentials() {
    return {
      appId: String(this.homey.settings.get('aqara_client_id') || '').trim(),
      appKey: String(this.homey.settings.get('aqara_client_secret') || '').trim(),
  ...
  async request(intent, data = {}, { accessToken = '' } = {}) {
    ...
    if (!appId || !appKey || !keyId) {
      throw new Error('Aqara Client ID, Client Secret and Key ID must be configured first.');
    }
