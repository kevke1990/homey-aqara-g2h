...
class AqaraApiClient extends EventEmitter {
  constructor({ clientId, clientSecret, accessToken, region = 'eu', timeoutMs = 15000 }) {
    ...
    const regionCode = REGIONS[region] || REGIONS.eu;
    this.baseUrl = `https://open-${regionCode}.aqara.com/v3.0/open/api`;
    this.wsUrl = `wss://open-${regionCode}.aqara.com/v3.0/open/ws`;
    this.ws = null;
  ...
  async request(intent, data = {}) {
    ...
    try {
      ...
      if (result.code !== 0) throw new Error(`Aqara API ${result.code}: ${result.message || 'Unknown error'}`);
  ...
  async getStreamUrl(subjectId) { return this.request('query.cam.stream.url', { subjectId }); }

  async setPrivacyMode(subjectId, enabled) {
    return this.request('write.resource.device.control', { subjectId, resources: [{ resourceId: 'privacy_mode', value: enabled ? 1 : 0 }] });
  }

  async setLightMode(subjectId, value) {
    return this.request('write.resource.device.control', { subjectId, resources: [{ resourceId: 'light_mode', value }] });
  }

  async setMicrophone(subjectId, enabled) {
    return this.request('write.resource.device.control', { subjectId, resources: [{ resourceId: 'microphone', value: enabled ? 1 : 0 }] });
  }

  async playAudioClip(subjectId, text) {
    return this.request('write.resource.device.control', { subjectId, resources: [{ resourceId: 'play_audio', value: text }] });
  }
