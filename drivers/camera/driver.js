...
class AqaraCameraDriver extends OAuth2Driver {
  ...
  _registerFlowCards() {
    this.homey.flow.getActionCard('set_privacy_mode').registerRunListener(async ({ device, enabled }) => device.setPrivacyMode(enabled === true || enabled === 'true'));
    this.homey.flow.getActionCard('play_audio_clip').registerRunListener(async ({ device, clip }) => device.playAudioClip(clip));
    this.homey.flow.getActionCard('export_recording').registerRunListener(async ({ device }) => device.exportRecording());
  }
  ...
  async onPairListDevices({ oAuth2Client }) {
    ...
    try {
      if (oAuth2Client) {
        ...
        const accountAuth = this.homey.app.getAqaraAccountAuth();
        if (!accountAuth.isAuthorized()) {
          throw new Error('Connect your Aqara account in the Aqara Cameras app settings first.');
        }
        devices = await accountAuth.getAllDevices();
      }
    } catch (error) {
    ...
    const cameras = devices
      ...
      .sort((a, b) => String(a.deviceName || a.did).localeCompare(String(b.deviceName || b.did), undefined, { sensitivity: 'base', numeric: true }));
